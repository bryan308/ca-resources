...
1. Construct a web page using `<form>` and `</form>` elements that integrates with backend services for data processing.
1. Develop an HTML form that incorporates multiple input types, each chosen to optimize data entry for specific user tasks.
1. Create an HTML form that uses a wide range of input attributes to provide advanced features.
1. Create and customize dropdown menus using the select and option elements to enhance user interactivity on web forms.
1. Implement and configure the textarea element and its attributes to enable multi-line text input in web forms.
1. Group and label related form controls using the fieldset and legend elements to improve form accessibility and organization.
1. Create and style various types of buttons using the button element, and understand their functions in form submission and user interaction.
</section>
<section>
### What is HTML Form?
...
> Adding the `multiple` attribute to `<input type="file">` allows users to select more than one file. 
...
|   Elements                                      |   Definition                                                                          |
|-------------------------------------------------|---------------------------------------------------------------------------------------|
| [`<form>`](#the-form-element)                   | Defines an HTML form for collecting user input.                                       |
| [`<label>`](#the-label-element)                 | Defines a label for several form elements.                                            |
| [`<input>`](#the-input-element)                 | The most important form element.                                                      |
| [`<select>`](#the-select-and-option-element)    | Defines a drop-down list.                                                             |
| [`<option>`](#the-select-and-option-element)    | Defines an option that can be selected.                                               |
| [`<datalist>`](#the-datalist-element)           | Specifies a list of pre-defined options for an `<input>` element.                     |
| [`<textarea>`](#the-textarea-element)           | Defines a multi-line text input field.                                                |
| [`<fieldset>`](#the-fieldset-and-legend-element)| Use to group related data in a form.                                                  |
| [`<legend>`](#the-fieldset-and-legend-element)  | Defines a caption for `<fieldset>` element.                                           |
| [`<button>`](#the-button-element)               | Use to create a clickable button, can be used to trigger custom Javascript functions. |
</SummaryTable>
</section>
<section>
<SummaryTable>
...
|------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------|
| [`action`](#html-form-attributes)                                | Defines the action to be performeed when the form is submitted.                                        |
| [`method`](#html-form-attributes)                                | Specifies the HTTP method used when submitting the form data. (`GET` or `POST`)                        |
| [`for`](#the-label-element)                                      | Use in label tag that should be equal to `id` attribute of the `<input>` element to bind them together.|
| [`type`](#the-input-element)                                     | Specifies the type of input element to display.                                                        |
| [`name`](#the-input-element)                                     | Use to reference elements in a JavaScript, or to reference for data after a form is submitted.         |
| [`multiple`](#input-types-url-time-and-file)                     | Allows users to select more than one file.                                                             |
| [`value`](#value-readonly-and-disabled)                          | Specifies the initial value for an input field.                                                        |
| [`readonly`](#value-readonly-and-disabled)                       | Specifies that the input field is read-only (cannot be changed).                                       |
| [`disabled`](#value-readonly-and-disabled)                       | Specifies that the input is disabled.                                                                  |
| [`size`](#the-size-and-maxlength)                                | Specifies the size (in characters) for the input field.                                                |
| [`maxlength`](#the-size-and-maxlength)                           | Specifies the maximum allowed length for the input field.                                              |
| [`min`](#the-min-max-and-step) and [`max`](#the-min-max-and-step)| Specify the minimum and maximum values for an `<input>` element.                                       |
| [`step`](#the-min-max-and-step)                                  | Specifies the legal number of intervals for an `<input>` element.                                      |
| [`placeholder`](#the-placeholder-required-and-autofocus)         | Specifies a hint describing an input field’s expected value.                                           |
| [`required`](#the-placeholder-required-and-autofocus)            | Specifies that an input field must be filled out before submitting the form.                           |
| [`autofocus`](#the-placeholder-required-and-autofocus)           | Specifes that an input field should automatically should focus when the page loads.                    |
| [`pattern`](#the-pattern-and-title)                              | Specifies a regular expression that the input element’s value is checked against.                      |
| [`title`](#the-pattern-and-title)                                | Use to describe the pattern to the user.                                                               |
| [`selected`](#the-select-option-elements)                        | Use to the `<option>` element to define a pre-selected option.                                         |
| [`list`](#the-datalist-element)                                  | Must be equal to the `id` attribute of `<datalist>` element.                                           |
| [`row`](#the-textarea-element)                                   | Specifies the visible number of lines in a text area.                                                  |
| [`cols`](#the-textarea-element)                                  | Specifies the visible width of a text area.                                                            |
</SummaryTable>
</section>
<section>
<SummaryTable>
|   Types                                         |   Definition                                                                                                                       |
|-------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------|
| [`text`](#input-types-text-and-password)        | Defines a one-line text input field.                                                                   |
| [`password`](#input-types-text-and-password)    | Defines a password field. The characters in password field are masked (shown as asterisk or circles).  |
| [`submit`](#input-types-submit-reset-and-button)| Defines a button for submitting form data to a form handler.                                           |
| [`reset`](#input-types-submit-reset-and-button) | Defines a reset button that will reset all form values to their default values.                        |
| [`button`](#input-types-submit-reset-and-button)| Defines a programmable button.                                                                         |
| [`radio`](#input-types-radio-and-checkbox)      | Lets a user to select only one of a limited number of choices.                                         |
| [`checkbox`](#input-types-radio-and-checkbox)   | Lets users select zero or more options for a limited number of choices.                                |
| [`email`](#input-types-email-number-and-date)   | Use for input fields that should contain an email address.                                             |
| [`number`](#input-types-email-number-and-date)  | Defines a numeric input field.                                                                         |
| [`date`](#input-types-email-number-and-date)    | Use for input fields that should contain a date.                                                       |
| [`range`](#input-types-range-tel)               | Defines a control for entering a number whose exact value is not important (slider control).           |
| [`tel`](#input-types-range-tel)                 | Use for input fields that should contain a telephone number.                                           |
| [`url`](#input-types-url-time-and-file)         | Use for input fields that should contain a URL address.                                                |
| [`time`](#input-types-url-time-and-file)        | Allows users to select time.                                                                           |
| [`file`](#input-types-url-time-and-file)        | Allows the user to choose a file from their device or computer’s storage.                              |
</SummaryTable>
</section>
<section>
### Credits
...
The form example is sourced from <MDXLink href="https://ui.shadcn.com?rel=ca-resources.vercel.app" target="_blank" rel="noopener noreferrer">shadcn/ui</MDXLink>.
</section>