...
<ol style={{listStyleType: 'lower-alpha'}}>
  <li>Describe the importance of HTML forms in a web page.</li>
  <li>Construct a web page using `<form>` and `</form>` elements that integrates with backend services for data processing.</li>
  <li>Develop an HTML form that incorporates multiple input types, each chosen to optimize data entry for specific user tasks.</li>
  <li>Create an HTML form that uses a wide range of input attributes to provide advanced features.</li>
  <li>Create and customize dropdown menus using the select and option elements to enhance user interactivity on web forms.</li>
  <li>Implement and configure the textarea element and its attributes to enable multi-line text input in web forms.</li>
  <li>Group and label related form controls using the fieldset and legend elements to improve form accessibility and organization.</li>
  <li>Create and style various types of buttons using the button element, and understand their functions in form submission and user interaction.</li>
</ol>
</section>
<section>
### What is HTML Form?
...
| `<input>`           | Defines various input controls in a form (e.g., text, password, submit).                            |
| `<select>`          | Defines a drop-down list.                                                                           |
| `<option>`          | Defines an option that can be selected.                                                             |
| `<datalist>`        | Provides autocomplete options for an `<input>` field.                                               |
...
| `<button>`          | Defines a clickable button, useful for form submission or other actions.                            |
</SummaryTable>
<SummaryTable>
...
| `type="password"`   | Defines a password input field, masking the entered characters.                                     |
| `type="submit"`     | Creates a button to submit the form data.                                                           |
| `type="reset"`      | Creates a button to reset form values to their default state.                                       |
| `type="button"`     | Defines a programmable button.                                                                      |
| `type="radio"`      | Creates radio buttons, allowing the user to select one option from a group.                         |
| `type="checkbox"`   | Creates checkboxes, allowing the user to select multiple options.                                   |
| `type="email"`      | Defines an input field for email addresses, with built-in validation.                               |
| `type="number"`     | Defines an input field for numeric data.                                                            |
| `type="date"`       | Defines an input field for date selection.                                                          |
| `type="range"`      | Defines a slider input for numeric range selection.                                                 |
| `type="file"`       | Allows the user to select files from their device.                                                  |
| `type="url"`        | Used for input fields that should contain a URL address.                                            |
...
| `readonly`          | Specifies that the form field is read-only, meaning its value cannot be changed.                    |
| `disabled`          | Disables a form element, making it unclickable and not submitted.                                   |
...
| `placeholder`       | Provides a hint about the expected input value in a field.                                          |
| `required`          | Specifies that the form element must be filled out before submission.                               |
| `autofocus`         | Automatically focuses a specific input field when the page loads.                                   |
| `pattern`           | Specifies a regular expression that the input element's value is checked against.                   |
| `title`             | Used to describe the pattern to the user.                                                           |
| `selected`          | Used to the `<option>` element to define a pre-selected option.                                     |
