...
function GitHubStarsButton({
  ...
  if (isLoading) {
    return (
      <motion.div
        className={cn(
          "text-primary-foreground h-10 rounded-lg bg-none px-4 py-2 opacity-50",
          className,
        )}
      ></motion.div>
    )
  ...
  return (
    <motion.a
      ...
      className={cn(
        ...
        "flex h-10 shrink-0 cursor-pointer items-center gap-2 px-4 py-2",
        // Typography
        ...
        // Dark mode variants
        "dark:bg-input/30 dark:border-input dark:hover:bg-input/50 dark:aria-invalid:ring-destructive/40",
        // SVG-specific styles
        "has-[>svg]:px-3 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-[18px]",
        // External classes
        className,
      )}
      {...props}
