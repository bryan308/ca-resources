...
const NumberDisplay = ({ motionValue, number, height, transition }: NumberDisplayProps) => {
...
const SlidingNumber = React.forwardRef<HTMLSpanElement, SlidingNumberProps>(
	(
		...
		newIntStr = padStart && newIntStr.length === 1 ? "0" + newIntStr : newIntStr

		const prevStr = prevNumberRef.current.toString()
		let [prevIntStr = ""] = prevStr.split(".")
		const [, prevDecStr = ""] = prevStr.split(".")
		prevIntStr = padStart && prevIntStr.length === 1 ? "0" + prevIntStr : prevIntStr
